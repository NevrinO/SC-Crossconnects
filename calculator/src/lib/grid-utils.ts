...
/**
 * Generate all number labels between start and end (inclusive)
 * Handles integer ranges with size limits to prevent DoS
 */
export function generateNumberRange(start: number, end: number, maxCount: number = 1000): number[] {
...
export function calculateGridBounds(room: Room): GridBounds {
  ...
  if (room.xyRange) {
    ...
    const numberLabels = generateNumberRange(startY, endY)

    // For now, keep default orientation (numbers on Y axis, letters on X axis)
    // Orientation support is handled at display level in components
  ...
  if (endpointYLabels.length >= 2) {
    yLabels.push(...generateNumberRange(endpointYLabels[0], endpointYLabels[endpointYLabels.length - 1]))
  } else {
